...
if [ "x$1" = "x" -o "x$2" = "x"  -o "x$3" = "x" -o "x$4" = "X" ]; then
  echo "USAGE: CPSparrow.sh <input CNF> <seed> <tempdir> <loadmodel>"
  exit 1
fi
...
satsolver=sparrowp						# name of the binary (if not in this directory, give relative path as well)
...
then 
	...
	then
		solveStart=`date +%s`
		python gen_one_predict.py -i $tmpCNF -o $predictFile -g 0 -l $loadModel
		...
		python move_back.py $model $realModel
