...
#define NEWN(P, N)                                  \
    do                                              \
    {                                               \
        (P) = yals_malloc(yals, (N) * sizeof *(P)); \
    } while (0)

#define DELN(P, N)                               \
    do                                           \
    {                                            \
        yals_free(yals, (P), (N) * sizeof *(P)); \
    } while (0)
...
void *yals_malloc(Yals *yals, size_t bytes)
{
    ...
    if (!res)
        yals_abort(yals, "out of memory in 'yals_malloc'");
...
void *yals_realloc(Yals *yals, void *ptr, size_t o, size_t n)
{
    ...
    if (!n)
    {
        yals_free(yals, ptr, o);
        return 0;
    }
    if (!o)
        return yals_malloc(yals, n);
    yals_dec_allocated(yals, o);
    res = yals->mem.realloc(yals->mem.mgr, ptr, o, n);
    if (n && !res)
        yals_abort(yals, "out of memory in 'yals_realloc'");
...
static unsigned yals_incsatcnt(Yals *yals, int cidx, int lit, int len)
{
    ...
    {
        if (res == 1)
            yals_dec_weighted_break(yals, yals->crit[cidx], len);
        else if (!res)
            yals_inc_weighted_break(yals, lit, len);
        yals->crit[cidx] ^= lit;
...
static unsigned yals_decsatcnt(Yals *yals, int cidx, int lit, int len)
{
    ...
    if (yals->crit)
    {
        ...
        if (res == 1)
            yals_inc_weighted_break(yals, other, len);
        else if (!res)
            yals_dec_weighted_break(yals, lit, len);
...
static unsigned yals_determine_weighted_break(Yals *yals, int lit)
{
    unsigned res;
    if (yals->crit)
        res = yals_dynamic_weighted_break(yals, lit);
    else
        res = yals_compute_weighted_break(yals, lit);
#ifndef NYALSTATS
...
static void yals_dequeue_queue(Yals *yals, int cidx)
{
    ...
    if (yals_need_to_defrag_queue(yals))
        yals_defrag_queue(yals);
}
...
static void yals_dequeue(Yals *yals, int cidx)
{
    ...
    assert_valid_cidx(cidx);
    if (yals->unsat.usequeue)
        yals_dequeue_queue(yals, cidx);
    else
        yals_dequeue_stack(yals, cidx);
}
...
static void yals_enqueue(Yals *yals, int cidx)
{
    ...
    assert_valid_cidx(cidx);
    if (yals->unsat.usequeue)
        yals_enqueue_queue(yals, cidx);
    else
        yals_enqueue_stack(yals, cidx);
}
...
static void yals_pick_assignment(Yals *yals, int initial)
{
    ...
    else
    {
        ...
        // {
        //     yals->vals[i] = ((rand() % 100 > 90) ? 1 : 0) ^ yals->network_predict[i-1];
        // }
        for(idx = 1; idx < nvars; idx++)
        {
            if(((yals_rand(yals) % 100 > 90) ? 1 : 0) ^ yals->network_predict[idx-1])
            {
                SETBIT(yals->vals, yals->nvarwords, idx);
            }
            else
...
static void yals_initopts(Yals *yals)
{
    OPTSTEMPLATE
}
                            ...
                            YalsMalloc m, YalsRealloc r, YalsFree f)
{
    ...
    if (!yals)
        yals_abort(0, "out-of-memory allocating manager in 'yals_new'");
...
{
    if (!lit)
        yals_abort(yals, "zero literal argument to 'yals_val'");
...
static void yals_print_strategy(Yals *yals, const char *type, int vl)
{
    ...
    STRATSTEMPLATE
    if (yals_is_default_strategy(yals))
...
void yals_load_predict(Yals *yals, const char* fileName, int V)
{
    yals->network_predict = (int*) malloc(sizeof(int) * (V));
	...
	fscanf(fpredict, "%d %d %lf", &predict_num_vars, &predict_num_clauses, &yals->predict_time);
	if(predict_num_vars != V)
	{
		printf("c Wrong Predcit File");
		exit(-1);
	}

	for(int thisi = 0; thisi < V; ++thisi)
		fscanf(fpredict, "%d", &yals->network_predict[thisi]);

	printf("p");
	for(int i = 0; i < V; ++i)
		printf(" %d", yals->network_predict[i]);
	printf("\n");
